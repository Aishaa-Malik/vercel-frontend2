  ...
  useEffect(() => {
    const fetchBookingUsage = async () => {
      ...
      try {
        ...
        if (usageError) {
          console.error('Error fetching booking usage:', usageError);
        } else if (usageData) {
          setBookingUsage({
            used: usageData.bookings_used || 0,
            total: usageData.bookings_limit || 25 // Default to 25 if not set
          });
        }
        ...
        {showUsageLimit && (
          <div className="mt-4">
            <div className="bg-blue-50 rounded-lg p-4">
              <h3 className="text-lg font-medium text-blue-800 mb-2">Monthly Booking Usage</h3>
              <p className="text-blue-600 mb-2">{bookingUsage.used} / {bookingUsage.total} Bookings used this month</p>
              <div className="w-full bg-gray-200 rounded-full h-2.5">
                <div 
                  className="bg-blue-600 h-2.5 rounded-full" 
                  style={{ width: `${(bookingUsage.used / bookingUsage.total) * 100}%` }}
                ></div>
              </div>
              <p className="text-right text-green-600 mt-1">{bookingUsage.total - bookingUsage.used} remaining</p>
            </div>
          </div>
        )}
      </div>

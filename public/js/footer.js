document.addEventListener("DOMContentLoaded", () => {
  ...
  const createParticles = () => {
    // Check if explosion container exists before trying to use it
    if (!explosionContainer) {
      console.warn('Explosion container not found, skipping particle creation');
      return;
    }
  ...
  const checkFooterPosition = () => {
    // Check if footer element exists before trying to get its bounding rect
    if (!footer) {
      console.warn('Footer element not found, skipping footer position check');
      return;
    }
  ...
  // Only create particles and start checking footer position if footer element exists
  if (footer) {
    createParticles();
    setTimeout(checkFooterPosition, 500);
  } else {
    console.warn('Footer element not found, footer animation disabled');
  }
});
